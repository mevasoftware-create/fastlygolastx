...
// Tüm tabloları oluşturan SQL - schema.ts'den türetilmiş
const createSQL = `
-- users tablosunu güncelle (zaten var, kolonları ekle)
ALTER TABLE users 
  MODIFY COLUMN role enum('user','admin','courier','business') NOT NULL DEFAULT 'user';
...
CREATE TABLE IF NOT EXISTS couriers (
  ...
  vehicleType enum('bicycle','motorcycle','car','van') NOT NULL DEFAULT 'motorcycle',
...
-- restaurantTransactions tablosu
...
console.log(`\n\nSonuç: ${applied} başarılı, ${errors} hata`);

// Tabloları kontrol et
const [tables] = await conn.execute('SHOW TABLES');
console.log('\nGüncel tablolar (' + tables.length + '):');
