...
export default function AreaPage() {
  ...
  const { data: area, isLoading } = trpc.areas.getBySlug.useQuery({ slug }, {
    enabled: !!slug,
    retry: false,
  ...
  const config = useMemo(() => 
    getAreaConfig(slug, language),
    [slug, language]
  );
  
  const areaName = useMemo(() => 
    content.name || content.title || slug,
    [content.name, content.title, slug]
  );
  ...
  const currentUrl = `${getCanonicalUrl()}/areas/${slug}`;
  ...
  if (isLoading) {
    ...
    const serviceText = language === 'tr' ? 'Hızlı teslimat hizmeti' : language === 'mk' ? 'Брза достава' : language === 'sq' ? 'Shërbim i shpejtë dorëzimi' : 'Fast delivery service';
    return (
      <>
        <SEOHead 
          title={`${formattedSlug} ${deliveryText} | FastlyGo`}
          description={`${serviceText} ${formattedSlug}, ${cityName}`}
            ...
            <section className="relative py-16 md:py-24 overflow-hidden">
              ...
              <div className="container relative">
                <div className="max-w-4xl mx-auto text-center space-y-6">
                  ...
                  <div className="h-8 bg-orange-100 rounded-lg w-48 mx-auto animate-pulse" />
  ...
  return (
    <>
      ...
      <SEOHead 
        title={seoData.title || content.title || `${areaName} ${language === 'tr' ? 'Teslimat' : language === 'mk' ? 'Достава' : language === 'sq' ? 'Dorëzim' : 'Delivery'}`}
        description={seoData.description || content.description || `${language === 'tr' ? 'Hızlı teslimat hizmeti' : language === 'mk' ? 'Брза достава' : language === 'sq' ? 'Shërbim i shpejtë dorëzimi' : 'Fast delivery service'} ${areaName}, ${language === 'tr' ? 'Üsküp' : language === 'mk' ? 'Скопје' : language === 'sq' ? 'Shkup' : 'Skopje'}`}
            ...
            <div className="container relative">
              <div className="max-w-4xl mx-auto text-center space-y-6">
                ...
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/80 backdrop-blur-sm text-orange-600 text-sm font-medium shadow-sm">
                  ...
                  {areaName} • {language === 'tr' ? 'Üsküp' : language === 'mk' ? 'Скопје' : language === 'sq' ? 'Shkup' : 'Skopje'}
                </div>
            ...
            <div className="container">
              <div className="text-center mb-12">
                ...
                <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                  {content.whyUsDescription || content.description || (language === 'tr' ? `${areaName} bölgesinde hızlı ve güvenilir teslimat hizmeti sunuyoruz` : language === 'mk' ? `Нудиме брза и сигурна достава во ${areaName}` : language === 'sq' ? `Ofrojmë dorëzim të shpejtë dhe të sigurt në ${areaName}` : `We offer fast and reliable delivery service in ${areaName}`)}
                </p>
              </div>
